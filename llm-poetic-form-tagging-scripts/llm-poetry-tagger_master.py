...
# LLM (GPT models and Cluade) Poetry Tagging Script 🤖 📚
...
openai_api_key = my_secrets.openai_api_key
...
poem_df['form'] = poem_df['form'].str.lower()
...
for model_choice in models:
  for prompt_number, prompt_dict in enumerate(constructed_prompts):
      # Feed prompt to ChatGPT
        print(f"Prompting ChatGPT {model_choice} with the prompt:\n✨{prompt_dict['prompt']}...✨\n\n")
        ...
        # Pull out specific parts of the ChatGPT answer — splitting on line breaks, and then taking whatever comes after the colon

        # In each case, we're also testing to see if there is a column or a line break, because in rare cases ChatGPT might produce an incorrectly formatted answer
        if len(answer.split("\n")) >= 4:
...
#------------------------------------------
# Feed Prompts to Claude and Dump to Google Sheet
...
#Here, we loop through a list of dictionaries with our constructed prompts, and feed Claude each prompt.

# Loop through models, loop through prompts
for model_choice in claude_models:
  for prompt_number, prompt_dict in enumerate(constructed_prompts):
        #import time
        #time.sleep(3.5)
        
      # Feed prompt to ChatGPT
        print(f"Prompting Claude {model_choice} with the prompt:\n✨{prompt_dict['prompt']}...✨\n\n")
        ...
        message = client.messages.create(
                    ...
                    messages=[
                        {"role": "user", 
                         # Limit input tokens
                        "content": prompt_dict['prompt'][:4090]}
                    ]
                )
        ...
        # In each case, we're also testing to see if there is a column or a line break, because in rare cases the LLM might produce an incorrectly formatted answer
        if len(answer.split("\n")) >= 4:
          for num in range(num_lines_answer):
            if "Form" in answer.split("\n")[num] and ":" in answer.split("\n")[num]:
              #print(answer.split("\n")[num])
